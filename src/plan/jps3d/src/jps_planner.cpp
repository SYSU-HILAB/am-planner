...
void JPSPlanner<Dim>::setParam(ros::NodeHandle &nh)
{
  ...
  sample_path_jps_pub_ =
      nh.advertise<visualization_msgs::Marker>("/sample_path_jps_vis", 1);
  map_update_timer =
      nh.createTimer(ros::Duration(1.0), &JPSPlanner::map_update_timer_cb, this);
}
...
{
  if (has_map) return;
  if (!map_util_set) return;
  if (!map_util_->has_map_()) return;
  updateMap();
  has_map = true;
...
template <int Dim>
void JPSPlanner<Dim>::setMapUtil(const std::shared_ptr<MapUtil<Dim>> &map_util)
{
  map_util_ = map_util;
  map_util_set = true;
}
...
template <int Dim>
vec_Vecf<Dim> JPSPlanner<Dim>::getSamplePath()
{
  return sample_path_;
}
template <int Dim>
...
template <int Dim>
bool JPSPlanner<Dim>::plan(const Vecf<Dim> &start, const Vecf<Dim> &goal, decimal_t eps,
                           bool use_jps)
{
  if (planner_verbose_)
  {
  ...
  {
    if (planner_verbose_) printf(ANSI_COLOR_RED "goal is not free!\n" ANSI_COLOR_RESET);
    status_ = 2;
    return false;
  }
  ...
  if (Dim == 3)
  {
    graph_search_ = std::make_shared<JPS::GraphSearch>(cmap_.data(), dim(0), dim(1),
                                                       dim(2), eps, planner_verbose_);
    graph_search_->plan(start_int(0), start_int(1), start_int(2), goal_int(0),
                        goal_int(1), goal_int(2), use_jps);
  }
  else
  {
    graph_search_ = std::make_shared<JPS::GraphSearch>(cmap_.data(), dim(0), dim(1), eps,
                                                       planner_verbose_);
  ...
  {
    if (planner_verbose_)
      std::cout << ANSI_COLOR_RED "Cannot find a path from " << start.transpose()
                << " to " << goal.transpose() << " Abort!" ANSI_COLOR_RESET << std::endl;
    status_ = -1;
    return false;
  }
